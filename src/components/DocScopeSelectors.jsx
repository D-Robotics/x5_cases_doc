...
/**
 * 文档页顶栏：版本选择；与 URL ?v=&p= 同步。
 * 使用自定义下拉而非原生 select，避免嵌入式预览等环境无法弹出原生菜单。
 */
export default function DocScopeSelectors() {
  const { version, product, setVersion, productMatrix } = useDocScopeFilter();
  const { i18n } = useDocusaurusContext();
  const isEnglish = i18n.currentLocale === 'en';

  const versions = useMemo(() => productMatrix[product] || [], [productMatrix, product]);
  const showVersionMenu = versions.length > 0;

  const versionValue = useMemo(
    () => (showVersionMenu && versions.includes(version) ? version : versions[0] || ''),
    [showVersionMenu, versions, version],
  );
  ...
  return (
    <div className="navbar__item doc-scope-selectors-nav-item">
